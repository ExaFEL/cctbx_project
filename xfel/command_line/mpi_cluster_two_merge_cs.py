...
if (__name__ == "__main__"):
  ...
  if rank == 0:
    for ix in xrange(len(tar_file_names)):
      #print "SCALER_WORKER_RANKREQ_xrange START RANK=%d TIME=%f TAR=%d"%(rank,tt(),ix)
      rankreq = comm.recv(source=MPI.ANY_SOURCE)
      #print "SCALER_WORKER_RANKREQ_xrange STOP RANK=%d TIME=%f TAR=%d"%(rank,tt(),ix)
      #print "SCALER_WORKER_RANKREQ_SEND START RANK=%d TIME=%f TAR=%d"%(rank,tt(),ix)
      comm.send(ix,dest=rankreq)
      #print "SCALER_WORKER_RANKREQ_SEND STOP RANK=%d TIME=%f TAR=%d"%(rank,tt(),ix)
    for rankreq in range(size-1):
      #print "SCALER_WORKER_RANKREQ_RECV START RANK=%d TIME=%f rankreq=%d"%(rank,tt(),rankreq)
      rankreq = comm.recv(source=MPI.ANY_SOURCE)
      #print "SCALER_WORKER_RANKREQ_RECV STOP RANK=%d TIME=%f rankreq=%d"%(rank,tt(),rankreq)
      #print "SCALER_WORKER_ENDRUN START RANK=%d TIME=%f rankreq=%d"%(rank,tt(),rankreq)
      comm.send('endrun',dest=rankreq)
      #print "SCALER_WORKER_ENDRUN STOP RANK=%d TIME=%f rankreq=%d"%(rank,tt(),rankreq)
    ...
    while True:
      ...
      #print "SCALER_WORKER_RANKSEND STOP RANK=%d TIME=%f"%(rank,tt())
      #print "SCALER_WORKER_IDXRECV START RANK=%d TIME=%f"%(rank,tt())
      idx = comm.recv(source=0)
      #print "SCALER_WORKER_IDXRECV STOP RANK=%d TIME=%f"%(rank,tt())

      if idx == 'endrun': 
        scaler_worker.finished_db_mgr = db_mgr
        break
      #print "SCALER_WORKER START RANK=%d TIME=%f TAR=%d"%(rank,tt(),idx)
      scaler_worker.tar_to_scale_frame_adapter(tar_list=[tar_file_names[idx],], db_mgr=db_mgr)
      #print "SCALER_WORKER STOP RANK=%d TIME=%f TAR=%d"%(rank,tt(),idx)

  #print "SCALER_WORKERS STOP RANK=%d TIME=%f"%(rank,tt())
  ...
  #print "GATHER STOP RANK=%d TIME=%f"%(rank,tt())

  if rank == 0:
    print "Processing reports from %d ranks"%(len(reports))
    ireport = 0
    for item in reports:
      #print "SCALER_M_ADD START RANK=%d TIME=%f"%(rank,tt())
      scaler_master._add_all_frames(item)
      #print "SCALER_M_ADD STOP RANK=%d TIME=%f"%(rank,tt())
    ...
    #print "MASTER_JOIN START RANK=%d TIME=%f"%(rank,tt())
    scaler_master.master_db_mgr.join() # database written, finalize the manager
    #print "MASTER_JOIN STOP RANK=%d TIME=%f"%(rank,tt())
    #print "MASTER_FINAL START RANK=%d TIME=%f"%(rank,tt())
    scaler_master.mpi_finalize()
    #print "MASTER_FINAL STOP RANK=%d TIME=%f"%(rank,tt())
